...
def manual_circuit(n_qubits, params):
    """
    Manually design parameterized quantum circuit with double excitation block.
    ...
    c = tc.Circuit(n_qubits)
    param_idx = 0
    # First Ry layer (n_qubits gates) - prepare superposition
    for i in range(n_qubits):
        c.ry(i, theta=params[param_idx])
        param_idx += 1
    # First CNOT ladder (n_qubits-1 gates) - initial entanglement
    for i in range(n_qubits - 1):
        c.cnot(i, i+1)  # parameter ignored
        param_idx += 1
    # Rz layer (n_qubits gates) - local phase adjustments
    for i in range(n_qubits):
        c.rz(i, theta=params[param_idx])
        param_idx += 1
    # Reverse CNOT ladder (n_qubits-1 gates) - map Pauli string to single-qubit Z
    for i in range(n_qubits - 2, -1, -1):
        c.cnot(i, i+1)  # parameter ignored
        param_idx += 1
    # Central Rz rotation on last qubit (n_qubits-1) - double excitation parameter
    c.rz(n_qubits - 1, theta=params[param_idx])
    param_idx += 1
    # Forward CNOT ladder again (n_qubits-1 gates) - restore mapping
    for i in range(n_qubits - 1):
        c.cnot(i, i+1)  # parameter ignored
        param_idx += 1
...
def optimize_vqe(h_qubit_op, n_qubits, n_params, init_params=None):
    """Optimize circuit parameters using BFGS."""
...
def circuit_gate_list(n_qubits, params):
    ...
    for i in range(n_qubits):
        gates.append(f"ry{i}")
        param_idx += 1
    # First CNOT ladder (n_qubits-1 gates)
    for i in range(n_qubits - 1):
    ...
    for i in range(n_qubits):
        gates.append(f"rz{i}")
        param_idx += 1
    # Reverse CNOT ladder (n_qubits-1 gates)
    for i in range(n_qubits - 2, -1, -1):
        gates.append(f"cnot{i}_{i+1}")
        param_idx += 1
    # Central Rz rotation on last qubit
    gates.append(f"rz{n_qubits-1}")
    param_idx += 1
    # Forward CNOT ladder again (n_qubits-1 gates)
    for i in range(n_qubits - 1):
        gates.append(f"cnot{i}_{i+1}")
        param_idx += 1
    # Ensure all parameters consumed
...
def main():
    ...
    results = {
        ...
        "circuit": {
            ...
            "design_rationale": "Double excitation ansatz with Ry and Rz rotations, plus CNOT ladders to implement four-qubit Pauli rotations (e.g., XXXX). Central Rz gate enables double excitation parameterization. Designed to capture correlation energy (~29 mHa) for 4-qubit LiH Hamiltonian."
        },
        "results": {
