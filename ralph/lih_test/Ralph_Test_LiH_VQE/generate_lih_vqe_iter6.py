...
def manual_circuit(n_qubits, params):
    """
    Manually design parameterized quantum circuit with double excitation block and phase correction.
    Assumes n_qubits = 4.
    Gate sequence (25 gates): X2, X3, Rz0, Rz1, Rz2, Rz3, H0, H1, H2, H3,
    CNOT0_1, CNOT1_2, CNOT2_3, Rz3 (double excitation), CNOT2_3, CNOT1_2, CNOT0_1,
    H0, H1, H2, H3, Ry0, Ry1, Ry2, Ry3.
    ...
    for i in range(3):
        c.cnot(i, i+1)
        param_idx += 1
    # Rz3 (double excitation parameter)
...
def optimize_vqe(h_qubit_op, n_qubits, n_params, init_params=None):
    ...
    if init_params is None:
        # Initialize parameters: phase correction at indices 2-5, double excitation at index 13, Ry at indices 21-24
        init_params = np.random.uniform(-0.1, 0.1, size=n_params)  # small random for all
        # Phase correction Rz angles (π/4 ≈ 0.785398) to convert imaginary coupling to real
        init_params[2:6] = 0.785398  # Rz0-3 phase correction
        # Double excitation angle from two-level system (1.7297 rad)
        init_params[13] = 1.7297  # double excitation angle
        # Ry angles small random
        init_params[21:25] = np.random.uniform(-0.1, 0.1, size=4)  # Ry angles
    else:
...
def main():
    ...
    # Circuit gate count: X(2) + Rz(4) + H(4) + CNOT(3) + Rz(1) + CNOT(3) + H(4) + Ry(4) = 25 gates
    # Each gate consumes a parameter, but fixed gates (X, H, CNOT) ignore their parameter.
    total_gates = 2 + 4 + 4 + 3 + 1 + 3 + 4 + 4  # 25 gates
    ...
    results = {
        ...
        "circuit": {
            ...
            "design_rationale": "Circuit prepares HF state |0011⟩, applies phase correction Rz(π/4) on each qubit to convert imaginary coupling to real, then double excitation block exp(iθ XXXX) via Hadamard+CNOT ladder, followed by Ry rotations to capture additional correlations. Designed to capture correlation energy (~29 mHa)."
        },
        "results": {
        ...
        "implementation_details": {
            ...
            "script_path": "generate_lih_vqe_iter6.py",
