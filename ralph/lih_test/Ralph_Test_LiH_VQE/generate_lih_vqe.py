...
Manual circuit design for LiH molecule at 2.0 Å bond length.
"""
...
np.random.seed(42)
# tc.set_random_seed(42)  # not available in this version
...
def manual_circuit(n_qubits, params):
    """
    Manually design parameterized quantum circuit.
    Uses tensorcircuit Circuit.
    Args:
        n_qubits: number of qubits
        params: 1D array of parameters (length n_params), one parameter per gate.
    Returns:
        circuit object
    """
    c = tc.Circuit(n_qubits)
    param_idx = 0
    # First Ry layer (n_qubits gates)
    for i in range(n_qubits):
        c.ry(i, theta=params[param_idx])
        param_idx += 1
    # CNOT ladder (n_qubits-1 gates)
    for i in range(n_qubits - 1):
        c.cnot(i, i+1)  # parameter ignored (CNOT has no parameter)
        param_idx += 1
    # Rz layer (n_qubits gates)
    for i in range(n_qubits):
        c.rz(i, theta=params[param_idx])
        param_idx += 1
    # Second Ry layer (n_qubits gates)
    for i in range(n_qubits):
        c.ry(i, theta=params[param_idx])
        param_idx += 1
    # Final CNOT ladder reverse (n_qubits-1 gates)
    for i in range(n_qubits - 2, -1, -1):
        c.cnot(i, i+1)  # parameter ignored
        param_idx += 1
    # No extra Rz layers (all parameters accounted for)
    # Ensure all parameters consumed
...
def optimize_vqe(h_qubit_op, n_qubits, n_params, init_params=None, n_restarts=3):
    """Optimize circuit parameters using BFGS with random restarts."""
    best_result = None
    best_energy = float('inf')
    best_energy_curve = None
    best_elapsed = 0.0
    ...
    for restart in range(n_restarts):
        if init_params is not None and restart == 0:
            ...
            params = np.random.uniform(-0.5, 0.5, size=n_params)

        energy_curve = []
        def callback(xk):
            energy = energy_function(xk, h_qubit_op, n_qubits)
            energy_curve.append(energy)
            print(f"Restart {restart+1}, Iteration {len(energy_curve)}: energy = {energy:.8f}")
        ...
        result = minimize(
            ...
            method='BFGS',
            callback=callback,
            options={'disp': False, 'maxiter': 500, 'gtol': 1e-8}
        )
        ...
        if result.fun < best_energy:
            ...
            best_result = result
            best_energy_curve = energy_curve.copy()
            best_elapsed = elapsed

        print(f"Restart {restart+1} finished: energy = {result.fun:.8f}, success = {result.success}")

    # Ensure energy curve includes final energy
    if len(best_energy_curve) == 0 or best_energy_curve[-1] != best_result.fun:
        best_energy_curve.append(best_result.fun)

    return best_result, best_energy_curve, best_elapsed

def circuit_gate_list(n_qubits, params):
    """
    Generate gate list representation for JSON output.
    Returns list of gate strings matching the manual circuit design.
    """
    gates = []
    param_idx = 0
    # First Ry layer (n_qubits gates)
...
def main():
    print("LiH VQE Custom Circuit Generation")
    print("="*50)
    ...
    total_gates = n_qubits * 3 + (n_qubits - 1) * 2  # Ry layers (n_qubits each) + Rz layer (n_qubits) + CNOT ladders (n_qubits-1 each)
    n_params = total_gates  # One parameter per gate (including zero parameters for CNOT)
    init_params = None  # Let optimize_vqe handle random restarts
    print(f"Starting VQE optimization with {n_params} parameters")
    result, energy_curve, opt_time = optimize_vqe(h_qubit_op, n_qubits, n_params, init_params, n_restarts=5)
    ...
    results = {
        ...
        "circuit": {
            ...
            "design_rationale": "Layered ansatz with single-qubit Ry/Rz rotations and CNOT ladders for entanglement. Designed to be expressive enough for 4-qubit LiH Hamiltonian."
        },
        "results": {
