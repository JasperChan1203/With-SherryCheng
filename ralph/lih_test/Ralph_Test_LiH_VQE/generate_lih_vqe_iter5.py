...
def manual_circuit(n_qubits, params):
    """
    Manually design parameterized quantum circuit with three Ry layers and two CNOT ladders.
    Total gates: n_qubits * 3 + (n_qubits - 1) * 2 = 18 for n_qubits=4.
    Each gate consumes one parameter (CNOT parameters ignored).
    """
    c = tc.Circuit(n_qubits)
    param_idx = 0
    ...
    for i in range(n_qubits):
        c.ry(i, theta=params[param_idx])
        param_idx += 1
    # CNOT ladder reverse (n_qubits-1 gates)
    for i in range(n_qubits - 2, -1, -1):
        c.cnot(i, i+1)
        param_idx += 1
    # Third Ry layer (n_qubits gates)
    for i in range(n_qubits):
        c.ry(i, theta=params[param_idx])
        param_idx += 1
...
def optimize_vqe(h_qubit_op, n_qubits, n_params, init_params=None, n_restarts=3):
    ...
    for restart in range(n_restarts):
        ...
        result = minimize(
            ...
            method='BFGS',
            callback=callback,
            options={'disp': False, 'maxiter': 100, 'gtol': 1e-8}
        )
...
def circuit_gate_list(n_qubits, params):
    ...
    Matches manual_circuit gate order.
    """
    gates = []
    param_idx = 0
    # First Ry layer (n_qubits gates)
...
def main():
    ...
    assert n_qubits == 4, f"Expected 4 qubits but got {n_qubits}"
    total_gates = 18  # Fixed gate count for our circuit design
    n_params = total_gates  # One parameter per gate (including zero parameters for CNOT, H)
    init_params = None  # Let optimize_vqe handle random restarts
    print(f"Starting VQE optimization with {n_params} parameters")
    result, energy_curve, opt_time = optimize_vqe(h_qubit_op, n_qubits, n_params, init_params, n_restarts=2)
    ...
    results = {
        ...
        "vqe_settings": {
            "ansatz_type": "manual_ry_cnot_layers",
            "optimizer": "BFGS",
        ...
        "circuit": {
            ...
            "design_rationale": "Three Ry layers interleaved with two CNOT ladders (Ry-CNOT-Ry-CNOT-Ry). Provides expressibility for 4-qubit LiH Hamiltonian with 18 gates, aiming to capture correlation energy (~29 mHa) through entangling rotations."
        },
        "results": {
        ...
        "implementation_details": {
            ...
            "script_path": "generate_lih_vqe_iter5.py",
