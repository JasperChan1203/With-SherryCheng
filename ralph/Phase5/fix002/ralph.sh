#!/bin/bash
# Ralph — RLQAS Phase 5 Fix 002: Genuine RL Circuit Search
# Fixes: (1) Phase1 MDP duplicate-action flaw; (2) Phase1 ent_coef=0.0; (3) Phase3 no-op training
# Usage: ./ralph.sh [--tool amp|claude] [max_iterations]

set -e

TOOL="claude"
MAX_ITERATIONS=15  # 4 stories, some need multiple iterations
...
if [ ! -f "$COST_LOG_FILE" ]; then
  echo "# RLQAS Phase 5 Fix 002 - Cost Log" > "$COST_LOG_FILE"
  echo "# Tool: $TOOL | Max iterations: $MAX_ITERATIONS" >> "$COST_LOG_FILE"
  echo "# Started: $(date)" >> "$COST_LOG_FILE"
  echo "---" >> "$COST_LOG_FILE"
fi

echo "Starting Ralph — Phase 5 Fix 002: Genuine RL Circuit Search"
echo "Tool: $TOOL | Max iterations: $MAX_ITERATIONS"
echo "Stories:"
echo "  US-001: Fix Phase 1 MDP — duplicate action terminates episode (HIGH)"
echo "  US-002: Fix Phase 1 ent_coef=0.0 default (HIGH)"
echo "  US-003: Verify Phase 1 LiH chemical accuracy (MEDIUM)"
echo "  US-004: Implement REINFORCE in Phase 3 HybridSearchController (MEDIUM)"
...
for i in $(seq 1 $MAX_ITERATIONS); do
  ...
  echo "  Phase 5 Fix 002: Genuine RL Circuit Search"
  echo "================================================================"
  ...
  if echo "$OUTPUT" | grep -q "<promise>COMPLETE</promise>"; then
    echo ""
    echo "Ralph completed all Phase 5 Fix 002 stories!"
