#!/bin/bash
# Ralph — RLQAS Phase 5 Bugfix 001
# Fixes: (1) Phase1 UCCSearchController PPO never trains; (2) Phase2 HEA best_energy=inf
# Usage: ./ralph.sh [--tool amp|claude] [max_iterations]

set -e

TOOL="claude"
MAX_ITERATIONS=10  # 2 focused bugfix stories
...
if [ ! -f "$COST_LOG_FILE" ]; then
  echo "# RLQAS Phase 5 Bugfix 001 - Cost Log" > "$COST_LOG_FILE"
  echo "# Tool: $TOOL | Max iterations: $MAX_ITERATIONS" >> "$COST_LOG_FILE"
  echo "# Started: $(date)" >> "$COST_LOG_FILE"
  echo "---" >> "$COST_LOG_FILE"
fi

echo "Starting Ralph — Phase 5 Bugfix 001"
echo "Tool: $TOOL | Max iterations: $MAX_ITERATIONS"
echo "Stories:"
echo "  US-001: Fix Phase 1 UCCSearchController PPO training (HIGH)"
echo "  US-002: Fix Phase 2 HEASearchController best_energy=inf (LOW)"
...
for i in $(seq 1 $MAX_ITERATIONS); do
  ...
  if echo "$OUTPUT" | grep -q "<promise>COMPLETE</promise>"; then
    echo ""
    echo "Ralph completed all Phase 5 Bugfix 001 stories!"
