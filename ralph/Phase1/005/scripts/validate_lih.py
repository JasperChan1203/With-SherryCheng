...
This script validates the complete RLQAS system using LiH molecule.
...
# Add Phase 1 task directories to Python path
TASK_DIRS = ["../001", "../002", "../003", "../004"]
for dir_path in TASK_DIRS:
    if os.path.exists(dir_path):
        sys.path.append(dir_path)
    else:
        raise ImportError(f"Required task directory not found: {dir_path}")
...
def check_module_health():
    ...
    try:
        # Test Task 001: Molecule processing
        ...
        if not hasattr(data, 'fci_energy'):
            errors.append("Task 001: MoleculeData missing fci_energy attribute")
    except Exception as e:
        errors.append(f"Task 001 (molecule processing) failed: {e}")

    try:
        # Test Task 002: Simulator creation
        from src.modules.quantum_simulator import SimulatorFactory
        simulator = SimulatorFactory.create_simulator(4)  # 4 qubits test
        if simulator is None:
            errors.append("Task 002: Simulator creation returned None")
    except Exception as e:
        errors.append(f"Task 002 (simulator creation) failed: {e}")

    try:
        # Test Task 003: RL agent creation
        from src.modules.rl_agents import PPOAgent
        agent = PPOAgent(config={'seed': 42, 'use_gpu': False})
        if agent is None:
            errors.append("Task 003: PPOAgent creation returned None")
    except Exception as e:
        errors.append(f"Task 003 (RL agent creation) failed: {e}")

    try:
        # Test Task 004: UCC search controller import
        from src.modules.ucc_search.controller import UCCSearchController
        # Just test import, instantiation requires molecule data
    except Exception as e:
        errors.append(f"Task 004 (UCC search module) failed: {e}")
...
def run_lih_validation(
    ...
    try:
        ...
        # 3. Create UCC search controller using Task 004 (which internally uses Task 003 RL agent)
        ...
        # 8. Generate report
        if generate_report and EVALUATION_MODULES_AVAILABLE:
            ...
            try:
                ...
                import traceback
                traceback.print_exc()
        elif generate_report and not EVALUATION_MODULES_AVAILABLE:
            print("  Skipping report generation (evaluation modules not available)")
...
def main():
    ...
    parser.add_argument('--transform', type=str, default='parity',
                        choices=['parity', 'jordan_wigner', 'bravyi_kitaev'],
                        help='Fermion-to-qubit transformation (default: parity)')
