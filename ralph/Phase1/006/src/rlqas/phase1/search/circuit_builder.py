...
class UCCCircuitBuilder:
    ...
    def __init__(self, molecule_data: MoleculeData, config: Dict[str, Any] = None):
        ...
        # Check if molecule_data already contains a UCCSD object (from processor)
        if hasattr(molecule_data, 'ucc_sd_object') and molecule_data.ucc_sd_object is not None:
            # Use the existing UCCSD object to ensure consistency
            self.ucc = molecule_data.ucc_sd_object
            # Verify that the UCCSD object matches the transform in molecular_info
            mol_info = molecule_data.molecular_info
            transform = mol_info.get("transform", "jordan_wigner")
            # Note: UCCSD uses Jordan-Wigner mapping internally
            # If transform is not 'jordan_wigner', there may still be inconsistency
            # but using the same UCCSD object ensures Hamiltonian-circuit consistency
        ...
        elif hasattr(molecule_data, 'ucc_object') and molecule_data.ucc_object is not None:
            # Fallback to generic UCC object (may not have excitation operators)
            ucc = molecule_data.ucc_object
            mol_info = molecule_data.molecular_info
            transform = mol_info.get("transform", "jordan_wigner")
            if transform != "jordan_wigner":
                warnings.warn(
            ...
            if hasattr(ucc, 'ex_ops') and hasattr(ucc, 'param_ids'):
                self.ucc = ucc
            else:
                # UCC object lacks excitation operators, need to create UCCSD from scratch
            ...
            # No UCC objects available, create UCCSD from scratch
            # Reconstruct PySCF molecule from molecular_info
            mol_info = molecule_data.molecular_info
            formula = mol_info.get("formula", "H2")
            bond_length = mol_info.get("bond_length_angstrom", 0.74)
    ...
    def build_circuit(self, excitations: List[Tuple[int, int]],
        ...
        if hasattr(circuit, "set_params"):
            ...
            circuit.params = params  # also store for reference
        else:
        ...
        # Attach UCCSD object for energy evaluation
        circuit.ucc = self.ucc
        return circuit
