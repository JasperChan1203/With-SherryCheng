#!/usr/bin/env python3
"""
LiH UCC GiGPO Learning Validation
====================================
Validates that the GiGPO agent in RLQAS achieves chemical accuracy on
LiH @ 1.6 A with active_space=(4,6) and max_excitations=5.

GiGPO (Group-relative Policy Optimization with intra-episode credit
assignment) computes per-step advantages via Monte Carlo returns with
anchor-group normalization, addressing sparse reward problems in QAS.
...
GiGPO runs via _grpo_search, so GRPODiagnosticsTracker works here.

Usage:
  python validate_lih_gigpo.py [--episodes 2000] [--output results/gigpo_lih_validation.json]
"""
...
def run_validation(n_episodes: int, output_path: str, diag_path: str):
    print("=" * 60)
    print("RLQAS LiH GiGPO Learning Validation")
    ...
    config = {
        ...
        'use_early_stop': True,
        # GiGPO hyperparameters (raw config keys read by controller)
        'lr': 3e-4,
        'clip_range': 0.2,
        'group_size': 8,
        'entropy_coef': 0.01,
    ...
    controller = UCCSearchController(mol, agent_type='gigppo', config=config)
    ...
    print("\n" + "=" * 60)
    print("GiGPO LEARNING DIAGNOSTICS SUMMARY")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print(f"  OVERALL: {'PASS -- GiGPO is learning' if overall else 'FAIL -- GiGPO may not be learning'}")
    print("=" * 60)
    ...
    print(f"GiGPO diagnostics -> {diag_path}")
...
def main():
    ...
    parser.add_argument('--output',   default='results/gigpo_lih_validation.json')
    parser.add_argument('--diag',     default='results/gigpo_lih_diagnostics.json')
