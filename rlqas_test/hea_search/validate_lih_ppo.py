#!/usr/bin/env python3
"""
LiH HEA PPO Learning Validation
=================================
Validates that the PPO agent in RLQAS can learn an HEA architecture that
...
Usage:
  python validate_lih_ppo.py [--episodes 500] [--output results/ppo_lih_hea_validation.json]
"""
...
def run_validation(n_episodes: int, output_path: str):
    print("=" * 60)
    print("RLQAS LiH HEA PPO Learning Validation")
    ...
    # n_steps must be <= total_timesteps for PPO
    n_steps = min(128, total_timesteps)
    ...
    print("\n" + "=" * 60)
    print("PPO HEA VALIDATION SUMMARY")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print(f"  OVERALL: {'PASS -- PPO is learning' if overall else 'FAIL -- PPO may not be learning'}")
    print("=" * 60)
...
def main():
    ...
    parser.add_argument('--output', default='results/ppo_lih_hea_validation.json')
