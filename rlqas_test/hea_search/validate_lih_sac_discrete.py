#!/usr/bin/env python3
"""
LiH HEA SAC-Discrete Learning Validation
==========================================
Validates that the SAC-Discrete agent in RLQAS can learn an HEA architecture
that achieves chemical accuracy on LiH @ 1.6 A with active_space=(2,3).
...
Usage:
  python validate_lih_sac_discrete.py [--episodes 500] [--output results/sac_discrete_lih_hea_validation.json]
"""
...
def run_validation(n_episodes: int, output_path: str):
    print("=" * 60)
    print("RLQAS LiH HEA SAC-Discrete Learning Validation")
    ...
    results = controller.search(
        ...
        agent_config={
            # SAC-Discrete does not accept n_steps
            'learning_rate': 3e-4,
            'buffer_size': 10000,
            'batch_size': 32,
            'learning_starts': 100,
    ...
    print("\n" + "=" * 60)
    print("SAC-DISCRETE HEA VALIDATION SUMMARY")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print(f"  OVERALL: {'PASS -- SAC-Discrete is learning' if overall else 'FAIL -- SAC-Discrete may not be learning'}")
    print("=" * 60)
...
def main():
    ...
    parser.add_argument('--output', default='results/sac_discrete_lih_hea_validation.json')
