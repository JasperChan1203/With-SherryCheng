#!/usr/bin/env python3
"""
LiH HEA GRPO Learning Validation
==================================
Validates that the GRPO agent in RLQAS can learn an HEA architecture that
achieves chemical accuracy on LiH @ 1.6 A with active_space=(2,3).

GRPO runs in groups of episodes rather than individual timesteps. Each group
produces a set of trajectories that are used for policy gradient updates with
...
Usage:
  python validate_lih_grpo.py [--episodes 500] [--output results/grpo_lih_hea_validation.json]
"""
...
def run_validation(n_episodes: int, output_path: str):
    ...
    print("\n" + "=" * 60)
    print("GRPO HEA VALIDATION SUMMARY")
    print("=" * 60)
...
def main():
    ...
    parser.add_argument('--output', default='results/grpo_lih_hea_validation.json')
