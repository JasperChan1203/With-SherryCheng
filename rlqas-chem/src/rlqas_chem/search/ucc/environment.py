...
class UCCSearchEnv(gym.Env):
    ...
    def __init__(self, molecule_data: MoleculeData, config: Dict[str, Any] = None):
        ...
        if self.encoding_method not in (None, "matrix"):
            try:
                from rlqas_chem.encoding.encoder_factory import EncoderFactory
    ...
    def get_valid_action_mask(self) -> np.ndarray:
        ...
        An action is invalid if it corresponds to an excitation already in the
        current circuit (duplicate) or if the circuit is already full.

        Returns:
            Boolean array of shape (n_actions,); True = valid, False = invalid.
        """
        max_depth = self.config.get("max_depth", 10)
        max_excitations = self.config.get("max_excitations", 20)
        circuit_full = (
            len(self.current_excitations) >= max_depth
            or len(self.current_excitations) >= max_excitations
        )
        if circuit_full:
            # No valid actions when circuit is already at capacity
            return np.zeros(self.n_actions, dtype=bool)

        mask = np.ones(self.n_actions, dtype=bool)
        for i, exc in enumerate(self.available_excitations):
            if exc in self.current_excitations:
                mask[i] = False
        return mask
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, Dict[str, Any]]:
        ...
        # Check max depth and max excitations BEFORE appending (fixes off-by-one)
        max_depth = self.config.get("max_depth", 10)
        max_excitations = self.config.get("max_excitations", 20)
        ...
        # With action masking, this should rarely occur. If it does (e.g. random
        # exploration ignoring the mask), penalise and terminate rather than crashing.
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[Dict[str, Any]] = None) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        # For 'hartree_fock' baseline, reset to HF energy every episode (correct).
        # For 'current_best' or 'rolling_average', preserve the cross-episode
        # tracking by only initialising on the very first call (when hf_energy is None).
        if self.reward_function.baseline_type == "hartree_fock":
            ...
            # First episode only: seed the HF reference so the reward function
            # has a valid fallback before the first compute_reward() call.
            self.reward_function.hf_energy = self._get_hf_energy()
        # Reset the per-episode first-evaluation flag so the reward function
        # correctly returns 0.0 on the first step of every new episode.
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        # Circuit parameters: show the parameters of the *selected* excitations
        # in selection order, padded to max_depth.
        # FIX: Previously we copied current_params[0:max_depth] (pool-order slice),
        # which made params for operators with index > max_depth-1 invisible to the
        # agent.  Now we pack the active parameters in the order they were chosen,
        # so all selected operators are always represented.
        params_padded = np.zeros(max_depth, dtype=np.float32)
        if self.current_params is not None and self.current_excitations:
    ...
    def _check_termination(self) -> bool:
        ...
        max_depth = self.config.get("max_depth", 10)
        max_excitations = self.config.get("max_excitations", 20)
        # max_steps caps the number of *steps taken* (including penalised invalid
        # actions), which is independent of the number of excitations added.
        max_steps = max(max_depth, max_excitations)

        # Max circuit depth / excitations reached (checked after appending)
        if len(self.current_excitations) >= max_depth:
            return True
        if len(self.current_excitations) >= max_excitations:
            return True

        # Safety cap: prevent infinite loops if many invalid actions are taken
        if self.step_count >= max_steps:
            return True

        # Energy convergence (optional, controlled by use_early_stop)
        if self._raw_config.get("use_early_stop", False) and self.molecule_data.fci_energy is not None:
    ...
    def _get_termination_reason(self) -> str:
        ...
        elif self._raw_config.get("use_early_stop", False) and self.molecule_data.fci_energy is not None:
            energy_error = abs(self.current_energy - self.molecule_data.fci_energy)
            if energy_error < self.config.get("convergence_threshold", 1.6e-3):
