...
class SACDiscreteAgent(RLAgent):
    ...
    def learn(
        ...
        Args:
            ...
            step_callback: Optional callable(step, env, losses) -> bool.
                Called every ``sample_freq`` steps. Return False to stop early.
            sample_freq: How often (in steps) to invoke step_callback.
        ...
        for t in range(total_timesteps):
            ...
            if (t >= learning_starts
                ...
                for _ in range(gradient_steps):
                    last_losses = self._update(batch_size)

            if step_callback is not None and t > 0 and t % sample_freq == 0:
                if step_callback(t, self.env, last_losses) is False:
                    break
    ...
    def _update(self, batch_size: int) -> Dict[str, float]:
        """One gradient update step (critic + actor + temperature).

        Returns:
            Dict with c1_loss, c2_loss, actor_loss, alpha.
        """
        ...
        return {
            "c1_loss": float(c1_loss.item()),
            "c2_loss": float(c2_loss.item()),
